<script lang="ts">
	...
	function compileMarkdown(markdown: string) {
		if (!markdown.trim()) {
			compiledHtml = '<p class="text-gray-500 italic">プレビューするコンテンツがありません</p>';
			return;
		}
	...
	function simpleMarkdownToHtml(markdown: string): string {
		...
		// Code blocks
		html = html.replace(/```([\s\S]*?)```/g, '<pre class="bg-gray-100 p-3 rounded-md overflow-x-auto my-4"><code class="text-sm">$1</code></pre>');
...
<div class="space-y-1">
	<div
		...
		class="border border-gray-300 rounded-md bg-white overflow-y-auto p-4 prose prose-sm max-w-none"
		style="height: {height};"
	>
		{@html compiledHtml}
	</div>
