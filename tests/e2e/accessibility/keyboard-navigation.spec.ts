...
test.describe('キーボードナビゲーションテスト', () => {
  ...
  test('フォームのキーボードナビゲーション', async ({ page }) => {
    ...
    if (formElementCount > 0) {
      ...
      // テキストフィールドでの文字入力テスト
      const textInputs = page.locator('form input[type="text"], form input[type="email"], form textarea');
  ...
  test('モーダルダイアログのキーボードナビゲーション', async ({ page }) => {
    ...
    // モーダルトリガーの存在確認
    const modalTriggers = page.locator('button[data-modal], [data-toggle="modal"], button:has-text("モーダル")');
    ...
    if (modalCount > 0) {
      ...
      if (await modal.isVisible()) {
        // フォーカストラップの確認
        const focusableInModal = modal.locator('button, input, textarea, select, a[href], [tabindex]:not([tabindex="-1"])');
  ...
  test('ページネーションのキーボードナビゲーション', async ({ page }) => {
    ...
    if (await pagination.isVisible()) {
      ...
      if (linkCount > 0) {
        ...
        // 次のページリンクを探す
        const nextPageLink = pagination.locator('a:has-text("次"), a:has-text("Next"), button:has-text("次")');
