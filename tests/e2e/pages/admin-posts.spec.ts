...
test.describe('管理画面 - 記事管理', () => {
  ...
  test('記事一覧ページが正常に表示される', async ({ page }) => {
    ...
    // 新規作成ボタンの確認
    await expect(page.locator('button:has-text("新規作成"), a:has-text("新規作成"), button:has-text("New"), a:has-text("New")')).toBeVisible();
  ...
  test('新しい記事を作成できる', async ({ page }) => {
    // 新規作成ボタンをクリック
    await page.click('button:has-text("新規作成"), a:has-text("新規作成"), button:has-text("New"), a:has-text("New")');
    await waitForPageLoad(page);
    ...
    // フォーム要素の確認
    await expect(page.locator('input[name="title"], [data-testid="title-input"]')).toBeVisible();
    await expect(page.locator('textarea[name="content"], [data-testid="content-input"]')).toBeVisible();
    
    // 記事データを入力
    await page.fill('input[name="title"], [data-testid="title-input"]', testContent.newPost.title);
    await page.fill('textarea[name="content"], [data-testid="content-input"]', testContent.newPost.content);
    
    // 概要（excerpt）フィールドがある場合
    const excerptField = page.locator('textarea[name="excerpt"], [data-testid="excerpt-input"]');
  ...
  test('記事を編集できる', async ({ page }) => {
    // 既存の記事を探して編集ボタンをクリック
    const editButton = page.locator('button:has-text("編集"), a:has-text("編集"), button:has-text("Edit"), a:has-text("Edit")').first();
    ...
    const contentTextarea = page.locator('textarea[name="content"], [data-testid="content-input"]');
    ...
    // 保存ボタンをクリック
    await page.click('button:has-text("保存"), button:has-text("Save"), button:has-text("更新"), button[type="submit"]');
    await waitForPageLoad(page);
  ...
  test('記事のステータスを変更できる', async ({ page }) => {
    ...
    const statusControl = page.locator('select[name*="status"], button:has-text("公開"), button:has-text("下書き")').first();
  ...
  test('記事の検索・フィルタ機能が動作する', async ({ page }) => {
    // 検索ボックスの確認
    const searchInput = page.locator('input[type="search"], input[placeholder*="検索"], [data-testid="search"]');
  ...
  test('記事のプレビュー機能が動作する', async ({ page }) => {
    // プレビューボタンの確認
    const previewButton = page.locator('button:has-text("プレビュー"), a:has-text("プレビュー"), button:has-text("Preview"), a:has-text("Preview")').first();
    ...
    if (await checkboxes.count() > 1) {
      ...
      if (await bulkAction.isVisible()) {
        if (await bulkAction.evaluate(el => el.tagName) === 'SELECT') {
          ...
          // 実行ボタンをクリック
          const executeButton = page.locator('button:has-text("実行"), button:has-text("Apply")');
  ...
  test('ページネーションが機能する', async ({ page }) => {
    ...
    if (await pagination.isVisible()) {
      const nextButton = pagination.locator('a:has-text("次"), a:has-text("Next"), button:has-text("次")');
  ...
  test('記事の複製機能が存在する場合の確認', async ({ page }) => {
    // 複製ボタンの確認
    const duplicateButton = page.locator('button:has-text("複製"), button:has-text("Duplicate"), a:has-text("複製")').first();
