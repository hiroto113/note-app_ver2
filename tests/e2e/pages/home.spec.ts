...
test.describe('ホームページ', () => {
  ...
  test('カテゴリフィルタが動作する', async ({ page }) => {
    // カテゴリフィルタの存在確認
    const categoryFilter = page.locator('select[id*="category"], select:has(option:text-contains("カテゴリ"))');
  ...
  test('ページネーションが動作する', async ({ page }) => {
    ...
    if (await pagination.isVisible()) {
      // 次のページボタンがあるかチェック
      const nextButton = pagination.locator('button:has-text("次"), button:has-text("›"), a:has-text("次")');
  ...
  test('ダークモードの切り替えが動作する', async ({ page }) => {
    // ダークモード切り替えボタンの確認
    const darkModeToggle = page.locator('button:has-text("Dark"), button:has-text("Light"), [data-testid="theme-toggle"]');
