...
test.describe('記事詳細ページ', () => {
	...
	test('ナビゲーション要素が機能する', async ({ page }) => {
		...
		const homeLink = page.locator(
			'a[href="/"], a:has-text("Home"), a:has-text("ホーム"), a:has-text("戻る")'
		);
	...
	test('共有機能が存在する場合の動作確認', async ({ page }) => {
		...
		const shareButtons = page.locator(
			'.share, .social-share, button:has-text("共有"), button:has-text("Share")'
		);
	...
	test('関連記事が表示される場合の確認', async ({ page }) => {
		...
		const relatedPosts = page.locator(
			'.related-posts, .similar-posts, section:has-text("関連記事")'
		);
