 ...
 * フォームの送信と結果の待機
 */
export async function submitFormAndWait(page: Page, formSelector: string, expectedUrl?: string | RegExp) {
 ...
 * 通知・トーストメッセージの確認
 */
export async function expectNotification(page: Page, message: string, type: 'success' | 'error' | 'info' = 'success') {
 ...
 * テーブル内の行を検索
 */
export async function findTableRow(page: Page, tableSelector: string, cellText: string): Promise<Locator> {
...
export async function closeModal(page: Page, closeSelector = 'button:has-text("閉じる"), button:has-text("Close"), [aria-label="Close"]') {
